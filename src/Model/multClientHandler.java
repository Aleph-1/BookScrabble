...
public class multClientHandler implements ClientHandler { // -1 - Illegal command, 0 - Too many players, 1 - Accepted, 2 - Update Board according to provided string, 3 -Not your turn, 4 - Next turn is id specified
    ...
    private void sendApproved(Word w) {
        ...
        out.println(2 + " " + "[" + w.getRow() + "," + w.getCol() + "," + w.isVertical() + "," + w.toStringW()+"]"); // 2 means update board, each customer will update his board according to that.
     /*   if(connections < 4)
            out.println(4 + " " + "Not enough users to determine whose turn next"); // At least 4 users are needed to know which turn's next.
        else
            out.println(4 + " " + users.get(connections % 4));

      */ //This part isn't needed until threads are introduced.
    ...
    public void handleClient(InputStream inFromclient, OutputStream outToClient) { //The host can be a player he just needs to connect to the server like regular clients.
        ...
        if (users.size() < 4 && !users.contains(intId))
            users.add(intId);
        else if (users.contains(intId) && users.get(connections % 4) != intId) { // If there's less than 4 connections and a user is already there.
